...
func (r *Reconciler) reconcile(
	...
	if !volumeGroup.DeletionTimestamp.IsZero() {
		...
		if added := controllerutil.AddFinalizer(volumeGroup, r.getFinalizer()); added {
			...
			return ctrl.Result{}, r.Update(ctx, volumeGroup)
		}
	}
	...
	bdi, err := r.BlockDeviceInfos(ctx, blockDevices)
	if err != nil {
	...
	// When explicit device paths are configured via DeviceSelector, the discovery policy
	// is not applicable — we always honor user-specified paths regardless of the policy.
	hasExplicitPaths := volumeGroup.Spec.DeviceSelector != nil &&
		(len(volumeGroup.Spec.DeviceSelector.Paths) > 0 || len(volumeGroup.Spec.DeviceSelector.OptionalPaths) > 0)

	effectivePolicy := ptr.Deref(volumeGroup.Spec.DeviceDiscoveryPolicy, lvmv1alpha1.DeviceDiscoveryPolicyStatic)
	...
	if !hasExplicitPaths && effectivePolicy == lvmv1alpha1.DeviceDiscoveryPolicyStatic && vgExists && len(devices.Available) > 0 {
...
func (r *Reconciler) determineFinishedRequeue(volumeGroup *lvmv1alpha1.LVMVolumeGroup, effectivePolicy lvmv1alpha1.DeviceDiscoveryPolicy) ctrl.Result {
	hasExplicitPaths := volumeGroup.Spec.DeviceSelector != nil &&
		(len(volumeGroup.Spec.DeviceSelector.Paths) > 0 || len(volumeGroup.Spec.DeviceSelector.OptionalPaths) > 0)
...
func (r *Reconciler) processDelete(ctx context.Context, volumeGroup *lvmv1alpha1.LVMVolumeGroup) error {
	...
	if !vgExistsInLVM {
		...
		if volumeGroup.Spec.ThinPoolConfig != nil {
			...
			thinPoolExists, err := r.LVExists(ctx, thinPoolName, volumeGroup.Name)
			if err != nil {
				return fmt.Errorf("failed to check existence of thin pool %q in volume group %q. %v", thinPoolName, volumeGroup.Name, err)
			}

			if thinPoolExists {
				if err := r.DeleteLV(ctx, thinPoolName, volumeGroup.Name); err != nil {
		...
		if err = r.DeleteVG(ctx, existingVG); err != nil {
	...
	if removed := controllerutil.RemoveFinalizer(volumeGroup, r.getFinalizer()); removed {
		...
		return r.Update(ctx, volumeGroup)
	}
	return nil
}
...
func (r *Reconciler) validateLVs(ctx context.Context, volumeGroup *lvmv1alpha1.LVMVolumeGroup) error {
	...
	resp, err := r.ListLVs(ctx, volumeGroup.Name)
	if err != nil {
	...
	for _, report := range resp.Report {
		...
		for _, lv := range report.Lv {
			...
			if lvAttr.State != StateActive {
				...
				err := r.ActivateLV(ctx, lv.Name, volumeGroup.Name)
				if err != nil {
...
func (r *Reconciler) addThinPoolToVG(ctx context.Context, vgName string, config *lvmv1alpha1.ThinPoolConfig) error {
	...
	if err := r.CreateLV(ctx, config.Name, vgName, config.SizePercent, convertChunkSize(config), convertMetadataSize(config)); err != nil {
...
func (r *Reconciler) extendThinPool(ctx context.Context, vgName string, lvSize string, config *lvmv1alpha1.ThinPoolConfig) error {
	...
	if err := r.ExtendLV(ctx, config.Name, vgName, config.SizePercent); err != nil {
...
func (r *Reconciler) verifyMetadataSize(ctx context.Context, vgName, lvName string, lvMetadataSize string, newMetadataSize int64) error {
	...
	err = r.ExtendThinPoolMetadata(ctx, lvName, vgName, newMetadataSize)
	if err != nil {
