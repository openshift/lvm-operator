...
for i in "${!all_y_streams[@]}"; do
    ...
    if [ "${i}" -lt ${#all_y_streams[@]} ] && [ "${i}" -gt 0 ]; then
        maxVersion="${all_y_streams[${i}-1]}"
    else
        maxVersion="${all_y_streams[${i}]}"
    fi

    # Filter out unsupported versions
    if ! [[ " ${TARGET_VERSIONS[*]} " =~ " ${minVersion} " ]]; then
        continue
    fi
    ...
    # Candidate versions
    catalog_versions=($(echo "${candidate_versions}" | yq "[.[] | select(contains(\"${maxVersion}\") or contains(\"${minVersion}\"))] | join(\" \")"))

    # Initialize the candidate channel array if there are candidates
    if (( ${#catalog_versions[@]} )) && [ -z "${SKIP_CANDIDATES+x}" ]; then
        catalog_template=$(echo "${catalog_template}" | yq '.Candidate.Bundles = []')

        for ver in "${catalog_versions[@]}"; do
            if ! [[ -n "${digests["${ver}"]}" ]]; then
                digests["${ver}"]=$(skopeo inspect "docker://${staging_bundle_path}:${ver}" --format "{{.Digest}}")
                echo "Pinning pre-release ${ver} to ${digests["${ver}"]}"
            fi

            catalog_template=$(echo "${catalog_template}" | yq ".Candidate.Bundles += ({\"Image\": \"${staging_bundle_path}@${digests["${ver}"]}\"} | (.Image | key) line_comment=\"${ver}\")")
        done
    elif ! [ -z "${SKIP_CANDIDATES+x}" ]; then
        echo "SKIP_CANDIDATES flag was set, skipping pre-release content"
    fi

    catalog_file_location="templates/lvm-operator-catalog-${minVersion}-template.yaml"
