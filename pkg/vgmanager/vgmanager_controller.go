...
var ConfigFilePath = "/mnt/lvmd/lvmd.yaml"
...
func (r *VGReconciler) reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	r.Log.V(1).Info("listing block devices")
	//  list block devices
	...
	for _, deviceClass := range lvmCluster.Spec.DeviceClasses {
		...
		if len(matchingDevices) > 0 {
			...
			if err != nil {
				r.Log.Error(err, "could not prepare volume groups")
				continue
			}
		}
	}
	...
	// todo(rohan): delete lvmd pod on this node
