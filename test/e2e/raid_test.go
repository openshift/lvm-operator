...
func raidTest() {
	...
	BeforeAll(func(ctx SpecContext) {
		...
		// Collect the union of discovered devices across every node, not just one.
		// Device names and signatures are not consistent across AWS nodes, so paths
		// discovered on a single node may resolve to a dirty or missing device on
		// another. OptionalPaths lets each node select whichever of these apply,
		// and every node must retain at least 2 clean devices for RAID1.
		devices := getDiscoveredDevicesAllNodes(ctx, cluster)
